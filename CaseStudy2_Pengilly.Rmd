...
# Having issues formatting test data as a matrix because it is missing the MonthlyIncome variable, which is required by the package
# Will continue working on, time-permitting.

# Remove unused variables in dataset and create dummy vars
```{r}
lasso_test_data = test_reg_data[,c(-1, -3, -10, -11, -23, -28)]
lasso_test_data = dummy_cols(lasso_test_data, remove_first_dummy = T)
lasso_test_data$MonthlyIncome = 0


test_x_vars = as.matrix(lasso_test_data)[,-1]
